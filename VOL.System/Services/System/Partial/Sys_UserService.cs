    ...
    public partial class Sys_UserService
    {
        ...
        public override PageGridData<Sys_User> GetPageData(PageDataOptions pageData)
        {
            ...
            QueryRelativeExpression = (IQueryable<Sys_User> queryable) =>
            {
                if (roleId <= 0)
                {
                    if (UserContext.Current.IsSuperAdmin) return queryable.Where(t=>t.User_Id != 1);
                ...
                return queryable.Where(x => roleIds.Contains(x.Role_Id) && x.User_Id != 1);
            };
            ...
            var object1 = base.GetPageData(pageData);
            return object1;
        }
